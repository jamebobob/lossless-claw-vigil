...
/** Strip em dashes (U+2014) from text before database insertion. */
function stripEmDash(value: string): string;
function stripEmDash(value: string | null | undefined): string | null | undefined;
function stripEmDash(value: string | null | undefined): string | null | undefined {
...
export class ConversationStore {
  ...
  async createMessage(input: CreateMessageInput): Promise<MessageRecord> {
    const result = this.db
      ...
      .run(input.conversationId, input.seq, input.role, stripEmDash(input.content), input.tokenCount);

    const messageId = Number(result.lastInsertRowid);

    this.indexMessageForFullText(messageId, stripEmDash(input.content));
  ...
  async createMessageParts(messageId: MessageId, parts: CreateMessagePartInput[]): Promise<void> {
    ...
    for (const part of parts) {
      stmt.run(
        ...
        part.partType,
        part.ordinal,
        stripEmDash(part.textContent) ?? null,
        part.toolCallId ?? null,
        part.toolName ?? null,
        stripEmDash(part.toolInput) ?? null,
        part.toolOutput ?? null,
        stripEmDash(part.metadata) ?? null,
      );
    }
  }
